<script setup>
const value = ref('')
const { loading, list, autocomplete, setInit } = useAutocompleteSearch()
...
<template>
	<div class="flex flex-row items-center relative" style="background-color: rgba(0, 0, 0, 0.13);">
		<input placeholder="Find movie" v-model="value" @input="autocomplete({ q: $event.target.value })"
		...
		<div :class="(list.length && value.length) ? 'block' : 'hidden'"
			...
			<NuxtLink v-for="row of list" class="font-semibold text-sm pb-4 cursor-pointer titel_1 h-9"
				:to="`/detail/${row?.id}`">
				<div @click="reset()">{{ row?.volumeInfo?.title }}</div>
			</NuxtLink>
		</div>
	</div>
</template>
